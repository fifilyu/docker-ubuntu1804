...
Ubuntu 18.04 LTS Docker镜像

## 构建镜像

```bash
git clone https://github.com/fifilyu/docker-ubuntu1804.git
cd docker-ubuntu1804
docker buildx build -t fifilyu/ubuntu1804:latest .
```
...
docker run -d \
    ...
    --name ubuntu1804 \
    fifilyu/ubuntu1804:latest
```

显示容器IP：

```bash
docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ubuntu1804
```
查看 `root` 用户随机密码：

```bash
docker logs ubuntu1804
```
